...
public class QueueAddServiceImplTest {

    @InjectMocks
    QueueAddServiceImpl addService;
    ...
    void addQueue() {
        ...
        when(repository.save(queuePO)).thenReturn(queuePO);
        when(mapper.map(queuePO)).thenReturn(queue);

        Queue result = addService.add(queue);

        assertEquals(queue, result);
        assertTrueLogEvent(INFO, "Player " + queue.getName() + " added to queue");
    }
